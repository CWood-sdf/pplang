...
template<typename left,typename right> using __apply = mergeArrays__actual <left, right>;
};
...
template<typename v> using __apply = sumAllArr__actual <v>;
};

template<typename v, typename... rest >
requires (GetValue<Bool<true>>::val)
struct sumAllArr__actual<Array<v, rest... >>{
using __ret = typename Add<v , typename sumAllArr::template __apply<Array<rest... >>::__ret >::__ret ;
...
using res  = typename sumAllArr::template __apply<typename mergeArrays::template __apply<arrayLeft , arrayRight >::__ret >::__ret ;

res  _____INSTANCE_OF_res ;

using printRes  = typename print::template __apply<res , Char<10> , Array<Int<1> , Int<2> , Int<3> >>::__ret ;
...
template<typename param> using __apply = addOne__actual <param>;
};
...
template<typename param> using __apply = subOne__actual <param>;
};
...
template<typename p1,typename p2> using __apply = twoParams__actual <p1, p2>;
};
...
template<typename param> using __apply = equalsOne__actual <param>;
};
...
template<typename param,typename value> using __apply = yeetWith__actual <param, value>;
};
...
template<typename asdf,typename otherFn,typename v> using __apply = yofn__actual <asdf, otherFn, v>;
};

using idrkanymore  = typename yofn::template __apply<addOne , yeetWith , Int<1> >::__ret ;

idrkanymore  _____INSTANCE_OF_idrkanymore ;

using idrkanymore2  = typename yofn::template __apply<subOne , yeetWith , Int<2> >::__ret ;

idrkanymore2  _____INSTANCE_OF_idrkanymore2 ;

template<typename param>
struct firstFunction__actual {
using __ret = typename Sub<param , typename Add<Float<(double)3> , typename Add<Float<(double)1> , typename Sub<typename otherFn::__apply::__ret , Float<(double)2> >::__ret >::__ret >::__ret >::__ret ;
...
template<typename param> using __apply = firstFunction__actual <param>;
};
...
template<typename param> using __apply = plus1__actual <param>;
};
...
template<typename param> using __apply = returnsFunction__actual <param>;
};
...
template<typename param> using __apply = returnsFnFn__actual <param>;
};
...
template<typename param2> using __apply = closure__actual <param2>;
};
...
template<typename param> using __apply = closureTest__actual <param>;
};
...
using returned2  = typename returnsFnFn::template __apply<returnsFunction >::__ret ;
...
template<typename param> using __apply = printParam__actual <param>;
};
...
template<typename... args> using __apply = sumAll__actual <args... >;
};

template<typename v, typename... rest >
requires (GetValue<Bool<true>>::val)
struct sumAll__actual<v, rest... >{
using __ret = typename Add<v , typename sumAll::template __apply<rest... >::__ret >::__ret ;
