...
struct otherFn {
  using __ret = Float<4.0>;
};
template <typename param> struct addOne {
  using __ret = typename Add<param, Int<1>>::__ret;
};
template <typename param> struct subOne {
  using __ret = typename Sub<param, Int<1>>::__ret;
};
template <typename p1, typename p2> struct twoParams {
  using __ret = typename Add<p1, p2>::__ret;
};
template <typename param> struct equalsOne {
  using __ret = typename Equals<param, Int<1>>::__ret;
};
template <template <typename> typename param, typename value> struct yeetWith {
...
template <template <typename> typename asdf,
          template <template <typename> typename, typename> typename otherFn,
          typename v>
struct yofn {
...
template <typename param> struct firstFunction {
  using __ret = typename Sub<
      param,
      typename Add<Float<3>,
                   typename Add<Float<1>, typename Sub<typename otherFn::__ret,
                                                       Float<2>>::__ret>::
                       __ret>::__ret>::__ret;
};
using yo = typename firstFunction<Float<1>>::__ret;
